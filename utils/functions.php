...
class Functions {
	 ...
	 * JSON Parse
	 */
	public static function json_parse( $stringfy, $default = array() ) {
		$out_put = '';
		try {
	 ...
	 * @return void
	 */
	public static function performance( $callback ) {
		// 記錄開始時間
		...
		// 取得 callback 名稱
		$callback_name = is_array( $callback ) ? get_class( $callback[0] ) . '::' . $callback[1] : $callback;

		// 輸出 callback 名稱與執行時間
		Utils::debug_log( "{$callback_name} 執行時間: {$execution_time} 秒" );
	}
}
